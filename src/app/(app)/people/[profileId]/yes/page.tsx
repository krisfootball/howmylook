...
type PublicYesPageProps = {
  params: Promise<{
    profileId: string;
  }>;
};

export default async function PublicYesPage({ params }: PublicYesPageProps) {
  ...
  if (profileError || !profile) {
    return (
      <MobileShell title="Yes given" subtitle="This profile could not be opened.">
  ...
  if (votesError) {
    return (
      <MobileShell title="Yes given" subtitle="This profile’s style history could not be loaded.">
        <section className="rounded-[1.6rem] border border-rose-100 bg-rose-50 p-5 text-sm leading-6 text-rose-700 shadow-sm">
          Unable to load this account’s yes votes right now.
        </section>
      </MobileShell>
    );
  }
  ...
  return (
    <MobileShell
      title="Yes given"
      subtitle={`${name}’s public yes votes. This helps people quickly understand the fashion looks and styling choices they naturally like.`}
    >
      <div className="space-y-4">
        ...
        <PublicVoteHistoryList items={items} value="yes" profileId={profileId} />
      </div>
    </MobileShell>
  );
}
